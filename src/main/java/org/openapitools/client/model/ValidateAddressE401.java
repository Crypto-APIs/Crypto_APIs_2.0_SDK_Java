...
public class ValidateAddressE401 extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(ValidateAddressE401.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ValidateAddressE401.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ValidateAddressE401' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<ValidateAddressE401>() {
                @Override
                public void write(JsonWriter out, ValidateAddressE401 value) throws IOException {
                ...
                @Override
                public ValidateAddressE401 read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        ValidateAddressE401 ret = new ValidateAddressE401();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonObject));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for ValidateAddressE401: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonObject.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public ValidateAddressE401() {
        super("oneOf", Boolean.FALSE);
    }

    public ValidateAddressE401(InvalidApiKey o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public ValidateAddressE401(MissingApiKey o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, GenericType> getSchemas() {
        return ValidateAddressE401.schemas;
    }
  ...
  * @throws IOException if the JSON Object is invalid with respect to ValidateAddressE401
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    ...
    if (validCount != 1) {
      throw new IOException(String.format("The JSON string is invalid for ValidateAddressE401 with oneOf schemas: InvalidApiKey, MissingApiKey. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonObj.toString()));
    }
  }

 /**
  * Create an instance of ValidateAddressE401 given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ValidateAddressE401
  * @throws IOException if the JSON string is invalid with respect to ValidateAddressE401
  */
  public static ValidateAddressE401 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ValidateAddressE401.class);
  }

 /**
  * Convert an instance of ValidateAddressE401 to an JSON string
  *
  * @return JSON string
  */
