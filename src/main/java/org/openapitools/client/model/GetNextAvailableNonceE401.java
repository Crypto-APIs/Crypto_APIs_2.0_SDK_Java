...
public class GetNextAvailableNonceE401 extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(GetNextAvailableNonceE401.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!GetNextAvailableNonceE401.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'GetNextAvailableNonceE401' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<GetNextAvailableNonceE401>() {
                @Override
                public void write(JsonWriter out, GetNextAvailableNonceE401 value) throws IOException {
                ...
                @Override
                public GetNextAvailableNonceE401 read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        GetNextAvailableNonceE401 ret = new GetNextAvailableNonceE401();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonObject));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for GetNextAvailableNonceE401: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonObject.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public GetNextAvailableNonceE401() {
        super("oneOf", Boolean.FALSE);
    }

    public GetNextAvailableNonceE401(InvalidApiKey o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public GetNextAvailableNonceE401(MissingApiKey o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, GenericType> getSchemas() {
        return GetNextAvailableNonceE401.schemas;
    }
  ...
  * @throws IOException if the JSON Object is invalid with respect to GetNextAvailableNonceE401
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    ...
    if (validCount != 1) {
      throw new IOException(String.format("The JSON string is invalid for GetNextAvailableNonceE401 with oneOf schemas: InvalidApiKey, MissingApiKey. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonObj.toString()));
    }
  }

 /**
  * Create an instance of GetNextAvailableNonceE401 given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of GetNextAvailableNonceE401
  * @throws IOException if the JSON string is invalid with respect to GetNextAvailableNonceE401
  */
  public static GetNextAvailableNonceE401 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GetNextAvailableNonceE401.class);
  }

 /**
  * Convert an instance of GetNextAvailableNonceE401 to an JSON string
  *
  * @return JSON string
  */
