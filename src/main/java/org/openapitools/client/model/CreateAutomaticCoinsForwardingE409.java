...
public class CreateAutomaticCoinsForwardingE409 extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(CreateAutomaticCoinsForwardingE409.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!CreateAutomaticCoinsForwardingE409.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'CreateAutomaticCoinsForwardingE409' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<CreateAutomaticCoinsForwardingE409>() {
                @Override
                public void write(JsonWriter out, CreateAutomaticCoinsForwardingE409 value) throws IOException {
                ...
                @Override
                public CreateAutomaticCoinsForwardingE409 read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        CreateAutomaticCoinsForwardingE409 ret = new CreateAutomaticCoinsForwardingE409();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonObject));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for CreateAutomaticCoinsForwardingE409: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonObject.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public CreateAutomaticCoinsForwardingE409() {
        super("oneOf", Boolean.FALSE);
    }

    public CreateAutomaticCoinsForwardingE409(AlreadyExists o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public CreateAutomaticCoinsForwardingE409(InvalidData o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, GenericType> getSchemas() {
        return CreateAutomaticCoinsForwardingE409.schemas;
    }
  ...
  * @throws IOException if the JSON Object is invalid with respect to CreateAutomaticCoinsForwardingE409
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    ...
    if (validCount != 1) {
      throw new IOException(String.format("The JSON string is invalid for CreateAutomaticCoinsForwardingE409 with oneOf schemas: AlreadyExists, InvalidData. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonObj.toString()));
    }
  }

 /**
  * Create an instance of CreateAutomaticCoinsForwardingE409 given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of CreateAutomaticCoinsForwardingE409
  * @throws IOException if the JSON string is invalid with respect to CreateAutomaticCoinsForwardingE409
  */
  public static CreateAutomaticCoinsForwardingE409 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, CreateAutomaticCoinsForwardingE409.class);
  }

 /**
  * Convert an instance of CreateAutomaticCoinsForwardingE409 to an JSON string
  *
  * @return JSON string
  */
