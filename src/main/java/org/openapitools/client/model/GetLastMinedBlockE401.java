...
public class GetLastMinedBlockE401 extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(GetLastMinedBlockE401.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!GetLastMinedBlockE401.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'GetLastMinedBlockE401' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<GetLastMinedBlockE401>() {
                @Override
                public void write(JsonWriter out, GetLastMinedBlockE401 value) throws IOException {
                ...
                @Override
                public GetLastMinedBlockE401 read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        GetLastMinedBlockE401 ret = new GetLastMinedBlockE401();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonObject));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for GetLastMinedBlockE401: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonObject.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public GetLastMinedBlockE401() {
        super("oneOf", Boolean.FALSE);
    }

    public GetLastMinedBlockE401(InvalidApiKey o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public GetLastMinedBlockE401(MissingApiKey o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, GenericType> getSchemas() {
        return GetLastMinedBlockE401.schemas;
    }
  ...
  * @throws IOException if the JSON Object is invalid with respect to GetLastMinedBlockE401
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
    ...
    if (validCount != 1) {
      throw new IOException(String.format("The JSON string is invalid for GetLastMinedBlockE401 with oneOf schemas: InvalidApiKey, MissingApiKey. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonObj.toString()));
    }
  }

 /**
  * Create an instance of GetLastMinedBlockE401 given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of GetLastMinedBlockE401
  * @throws IOException if the JSON string is invalid with respect to GetLastMinedBlockE401
  */
  public static GetLastMinedBlockE401 fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, GetLastMinedBlockE401.class);
  }

 /**
  * Convert an instance of GetLastMinedBlockE401 to an JSON string
  *
  * @return JSON string
  */
